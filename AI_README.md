...
*   **核心技术栈**: Python 3, Typer (CLI 框架), Rich (终端 UI), Volcengine SDK (ASR/LLM 服务)。
...
*   **`src/main.py`**: 应用程序入口点。
    *   定义 Typer 命令：`init` (初始化), `record` (记录), `analyze` (分析)。
...
```json
{
  ...
  "key_points": ["..."],         // 关键点列表
  "action_items": ["..."],       // 待办事项列表
  "sentiment": "String",         // 客户情感分析
  "created_at": "ISO-8601 Timestamp" // 创建时间戳
}
```
